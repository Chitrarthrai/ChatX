...
import { UserStatus } from "@chatx/types";
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0">
            {(["all", "online", "away", "dnd", "offline"] as const).map((st) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-medium capitalize transition-all whitespace-nowrap ${roleFilter === st
        ...
        {loading ? (
            ...
            {[1, 2, 3, 4, 5, 6].map((n) => (
              <div key={n} className="bg-card/50 border border-border p-5 rounded-2xl space-y-4 animate-pulse">
                ...
                <div className="pt-2 border-t border-border flex gap-2">
                  <div className="h-8 bg-secondary rounded-xl flex-1" />
          ...
          <div className="flex flex-col items-center justify-center p-12 bg-card/30 border border-dashed border-border rounded-3xl text-center space-y-4">
            ...
            <div className="space-y-1 max-w-sm">
              <h3 className="font-semibold text-sm">No contacts found</h3>
              <p className="text-xs text-muted-foreground">
                {search
                  ? `No team member matches "${search}". Try clearing search.`
